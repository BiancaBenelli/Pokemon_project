...
if st.sidebar.checkbox("Plots"):
    ...
    if plot == 'Ability 1 frequency':
        st.write('I do as I did for the columns Type 1 and Type 2:')
        fig = plt.figure(figsize = (15, 10))
        ...
        # DA SISTEMARE
        st.write('Now  frequency of Ability 1 associated with Types 1 with a Barchart:')
        type_2_present_mask = pokemon_df[pokemon_df['type_2'] != 'None']
        type_list = type_2_present_mask['type_1'].value_counts().index
    ...
    if plot == 'Ability 2 frequency':
        st.write('I do as I did for Ability 1:')
        ...
        for i, ax in zip(range(len(type_list)), axs.flat):
            ...
            ax.tick_params(axis = 'both', labelsize = 15)
            ax.set_title(abil_list[i] + " ability", fontsize = 20, fontweight='bold')
        st.write(fig)

    if plot == 'Strongest Pokemon':
        st.write(2)
        strongest_pokemon = pokemon_df['total_points'].max()
        st.write('The strongest Pokemon are: ', strongest_pokemon)
        weakest_pokemon = pokemon_df['total_points'].min()
        st.write('The weakest Pokemon is: ', weakest_pokemon)
        plt.figure(figsize = (15, 15))
        pokemon_df.hist(column = 'total_points', figsize=[12, 8], color = "green", bins=100, grid=False)
        plt.title('Pokemon by Total points')
        ...
        fig =  plt.figure(figsize = (10, 8))
        plt.title('10 strongest Pokemon')
        ...
        if st.checkbox("Do you want to see other plots that could be used?"):
            ...
            st.write('If I focus on the "special" Pokemon:')
            special_mask = pokemon_df[pokemon_df['status'] != 'Normal']
